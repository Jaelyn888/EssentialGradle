...
public class ShopCartFragment extends LazyFragment {
    ...
    public void setEnsure_order_tv(double money) {
        try {
            proTotleMoney = money;
            totleMoney = StringUtils.getFormatMoneyWithSignMust(proTotleMoney);
            // totleMoney = "0.00";
    ...
    private void setTotalMoney(String money) {
        String totle_money_formart = String.format(getApplicationContext().getString(R.string.totle_money), money);
    ...
    private void selectBrandSetTotalMoney(List<JsonMap<String, Object>> datas, int position) {
        ...
        if (datas.size() > 0) {
            for (int j = 0; j < datas.size(); j++) {
                List<JsonMap<String, Object>> shopcarts = datas.get(j).getList_JsonMap("skuShopCartVo");
                for (int k = 0; k < shopcarts.size(); k++) {
                    ...
                    if (shopcartPro.getStringNoNull("activityPrices").equals("") ||
                            shopcartPro.getStringNoNull("activityPrices").equals(shopcartPro.getDouble("prices"))) {
                        price = shopcartPro.getDouble("prices");
                    } else {
                        price = shopcartPro.getDouble("activityPrices");
                    }
                    ...
                    if (shopcartPro.getInt("isSelected") == 1) {
                        if (shopcartPro.getInt("saleStatus") == 1 && shopcartPro.getInt("productSkuIsDeleted") != 1
                            ...
                            money += price * shopcartPro.getInt("productCount");
