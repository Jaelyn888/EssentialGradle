...
import com.yishanxiu.yishang.model.shopmall.AllBrandListResponse;
...
public class BrandAllFragment extends LazyFragment {
	...
	private ArrayList<BrandSortModel> sourceDataList = new ArrayList<>();

	//热门品牌
	private List<BrandModel> hotBrandList = new ArrayList<>();
	//品牌搜索引擎
		...
		public void onLoaded(GetDataQueue entity) {
			if (entity.getWhat() == GetDataConfing.What_SelectBrandList) {
				...
				if (ShowGetDataError.isCodeSuccess(activity, baseResponse)) {
					...
					hotBrandList =  brandListResponse.getHotBrandList();
					sourceDataList = filledData((ArrayList<BrandSortModel>) brandListResponse.getTotalBrandList());
					initViews();
					validateData();
				}
			}
	 ...
	 * @return
	 */
	private ArrayList<BrandSortModel> filledData(ArrayList<BrandSortModel> date) {
		for (BrandSortModel brandSortModel : date) {
			// 汉字转换成拼音
			String pinyin = characterParser.getSelling(brandSortModel.getBrandName());
			try {
				String sortString = pinyin.substring(0, 1).toUpperCase();
				// 正则表达式，判断首字母是否是英文字母
				if (sortString.matches("[A-Z]")) {
					brandSortModel.setSortLetters(sortString.toUpperCase());
				} else {
					brandSortModel.setSortLetters(SideBar.b[SideBar.b.length]);
				}
	...
	private void initViews() {
		...
		xlv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (position > 0) {
					BrandSortModel BrandSortModel = sourceDataList.get(position - 1);
					activity.seeShopDetail(BrandSortModel);
				}
			}
		});
	...
	private void initHotViews() {
		if (hotBrandList.isEmpty()) {
			...
			hot_brand_gv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					BrandModel brandData = hotBrandList.get(position);
