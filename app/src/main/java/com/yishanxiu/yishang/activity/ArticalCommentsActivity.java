...
public class ArticalCommentsActivity extends BaseUIActivity {
	 ...
	 * 平论
	 */
	private List<ArticleCommentModel> dataList = new ArrayList<>();
	...
	private IGetServicesDataCallBack callBack = new IGetServicesDataCallBack() {
		...
		public void onLoaded(GetDataQueue entity) {
			if (entity.what == GetDataConfing.What_GetArticalCommentsList
				...
				TypeToken<BaseResponse<List<ArticleCommentModel>>> typeToken=new TypeToken<BaseResponse<List<ArticleCommentModel>>>(){};
				BaseResponse<List<ArticleCommentModel>> baseResponse=new ModleUtils().mapTo(entity,typeToken);
				if (ShowGetDataError.isCodeSuccess(mContext, baseResponse)) {

					List<ArticleCommentModel> temp_data=baseResponse.getInfo();
					if (temp_data.size() > 0) {
	...
	private void setAdapterData(List<ArticleCommentModel> temp_data, int requestType) {
		if (requestType == 0) {
			if (temp_data.size() == 0) {
