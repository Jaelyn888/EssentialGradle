...
/**
 * Created by Jaelyn on 2016/9/28.
 */
...
public class PullToZoomView extends ScrollView {
	...
	private float sensitive = 1.8f;         //放大的敏感系数
	...
	private LinearLayout.LayoutParams headerParams;//头部的参数
	...
	private PullToZoomView.OnScrollListener scrollListener;  //滚动的监听

	public void setOnScrollListener(PullToZoomView.OnScrollListener scrollListener) {
	...
	private PullToZoomView.OnPullZoomListener pullZoomListener; //下拉放大的监听

	public void setOnPullZoomListener(PullToZoomView.OnPullZoomListener pullZoomListener) {
	...
	public PullToZoomView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		sensitive = a.getFloat(R.styleable.PullToZoomView_sensitive, sensitive);
		isParallax = a.getBoolean(R.styleable.PullToZoomView_isParallax, isParallax);
		isZoomEnable = a.getBoolean(R.styleable.PullToZoomView_isZoomEnable, isZoomEnable);
		zoomTime = a.getInt(R.styleable.PullToZoomView_zoomTime, zoomTime);
		int zoomViewResId = a.getResourceId(R.styleable.PullToZoomView_zoomView, 0);
		if (zoomViewResId > 0) {
			zoomView = mLayoutInflater.inflate(zoomViewResId, null, false);
		}

		int headerViewResId = a.getResourceId(R.styleable.PullToZoomView_headerView, 0);
		if (headerViewResId > 0) {
			headerView = mLayoutInflater.inflate(headerViewResId, null, false);
		}
		int contentViewResId = a.getResourceId(R.styleable.PullToZoomView_contentView, 0);
		if (contentViewResId > 0) {
			contentView = mLayoutInflater.inflate(contentViewResId, null, false);
		}
		a.recycle();
	...
	protected void onLayout(boolean paramBoolean, int paramInt1, int paramInt2,
		...
		if (headerHeight == 0 && zoomView != null) {
			headerHeight = mHeaderContainer.getHeight();
		}
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		//findTagViews(this);
		if (mHeaderContainer!=null) {
			headerParams = (LinearLayout.LayoutParams) mHeaderContainer.getLayoutParams();
	...
	private void findTagViews(View v) {
		...
		if (zoomView != null) {
			mHeaderContainer.addView(zoomView);
		}
		if (headerView != null) {
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		if (isParallax) {
			if (t >= 0 && t <= headerHeight) {
				mHeaderContainer.scrollTo(0, -(int) (0.65 * t));
			} else {
