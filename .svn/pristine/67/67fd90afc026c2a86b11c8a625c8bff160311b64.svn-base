...
public class BitmapHelper {
		...
		switch (type) {
			case Rectangle:
				if (StringUtils.isEmptyNoNullWithTrimend(pUrl, true)) {
					...
					requestCreator = picasso.load(Uri.parse(pUrl));
				}
	...
	private static void glideLoad(Context context, String pUrl, ImageView imageView, LoadImgOption type, boolean isWeakCache, ImageLoaderBitmapCallBack callBack) {
		...
		switch (type) {
			case Rectangle:
				if (StringUtils.isEmptyNoNullWithTrimend(pUrl, true)) {
					...
					drawableTypeRequest=requestManager.load(Uri.parse(pUrl));
				}
		...
		drawableTypeRequest.asBitmap().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(imageView);
